...
public class AsmHook implements Cloneable, Comparable<AsmHook> {
   ...
   protected void createMethod(HookInjectorClassVisitor classVisitor) {
      ...
      MethodVisitor mv = classVisitor.visitMethod(1, superMethod == null ? this.targetMethodName : superMethod.name, this.targetMethodDescription, (String)null, (String[])null);
   ...
   private void injectLoad(HookInjectorMethodVisitor inj, Type parameterType, int variableId) {
      ...
      if (parameterType != Type.INT_TYPE && parameterType != Type.BYTE_TYPE && parameterType != Type.CHAR_TYPE && parameterType != Type.BOOLEAN_TYPE && parameterType != Type.SHORT_TYPE) {
         if (parameterType == Type.LONG_TYPE) {
            opcode = 22;
   ...
   private void injectSuperCall(HookInjectorMethodVisitor inj, ClassMetadataReader.MethodReference method) {
      ...
      for(int i = 0; i <= this.targetMethodParameters.size(); ++i) {
         Type parameterType = i == 0 ? TypeHelper.getType(this.targetClassName) : (Type)this.targetMethodParameters.get(i - 1);
   ...
   private void injectReturn(HookInjectorMethodVisitor inj, Type targetMethodReturnType) {
      if (targetMethodReturnType != Type.INT_TYPE && targetMethodReturnType != Type.SHORT_TYPE && targetMethodReturnType != Type.BOOLEAN_TYPE && targetMethodReturnType != Type.BYTE_TYPE && targetMethodReturnType != Type.CHAR_TYPE) {
         if (targetMethodReturnType == Type.LONG_TYPE) {
            inj.visitInsn(173);
   ...
   private void injectInvokeStatic(HookInjectorMethodVisitor inj, int returnLocalId, String name, String desc) {
      for(int i = 0; i < this.hookMethodParameters.size(); ++i) {
         Type parameterType = (Type)this.hookMethodParameters.get(i);
         int variableId = (Integer)this.transmittableVariableIds.get(i);
         if (inj.isStatic) {
            if (variableId == 0) {
               inj.visitInsn(1);
               continue;
   ...
   public static AsmHook.Builder newBuilder() {
   ...
   public class Builder extends AsmHook {
      ...
      public AsmHook.Builder setTargetClass(String className) {
         AsmHook.this.targetClassName = className;
         return this;
      }

      public AsmHook.Builder setTargetMethod(String methodName) {
         AsmHook.this.targetMethodName = methodName;
         return this;
      }

      public AsmHook.Builder addTargetMethodParameters(Type... parameterTypes) {
         Type[] var2 = parameterTypes;
      ...
      public AsmHook.Builder addTargetMethodParameters(String... parameterTypeNames) {
      ...
      public AsmHook.Builder setTargetMethodReturnType(Type returnType) {
         AsmHook.this.targetMethodReturnType = returnType;
         return this;
      }

      public AsmHook.Builder setTargetMethodReturnType(String returnType) {
         return this.setTargetMethodReturnType(TypeHelper.getType(returnType));
      }

      public AsmHook.Builder setHookClass(String className) {
         AsmHook.this.hooksClassName = className;
         return this;
      }

      public AsmHook.Builder setHookMethod(String methodName) {
         AsmHook.this.hookMethodName = methodName;
         return this;
      }

      public AsmHook.Builder addHookMethodParameter(Type parameterType, int variableId) {
      ...
      public AsmHook.Builder addHookMethodParameter(String parameterTypeName, int variableId) {
         return this.addHookMethodParameter(TypeHelper.getType(parameterTypeName), variableId);
      }

      public AsmHook.Builder addThisToHookMethodParameters() {
         if (!AsmHook.this.hasHookMethod()) {
            throw new IllegalStateException("Hook method is not specified, so can not append parameter to its parameters list.");
         } else {
      ...
      public AsmHook.Builder addReturnValueToHookMethodParameters() {
         if (!AsmHook.this.hasHookMethod()) {
            throw new IllegalStateException("Hook method is not specified, so can not append parameter to its parameters list.");
      ...
      public AsmHook.Builder setReturnCondition(ReturnCondition condition) {
      ...
      public AsmHook.Builder setReturnValue(ReturnValue value) {
      ...
      public AsmHook.Builder setPrimitiveConstant(Object constant) {
      ...
      public AsmHook.Builder setReturnMethod(String methodName) {
      ...
      public AsmHook.Builder setInjectorFactory(HookInjectorFactory factory) {
         AsmHook.this.injectorFactory = factory;
         return this;
      }

      public AsmHook.Builder setPriority(HookPriority priority) {
         AsmHook.this.priority = priority;
         return this;
      }

      public AsmHook.Builder setCreateMethod(boolean createMethod) {
         AsmHook.this.createMethod = createMethod;
         return this;
      }

      public AsmHook.Builder setMandatory(boolean isMandatory) {
         AsmHook.this.isMandatory = isMandatory;
         return this;
      }

      private String getMethodDesc(Type returnType, List<Type> paramTypes) {
         Type[] paramTypesArray = (Type[])paramTypes.toArray(new Type[0]);
         if (returnType == null) {
